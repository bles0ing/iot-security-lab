...
void HAL_UART_MspInit(UART_HandleTypeDef* uartHandle)
{
    ...
    if (uartHandle->Instance == USART1)
    {
		...
		if(uartHandle->Instance==USART3)
		{
			...
			/* 配置 USART3 TX 引脚：PB10 → USART3_TX */
			GPIO_InitStruct.Pin = RS485_TX_Pin;           // 选择 PB10 引脚
			GPIO_InitStruct.Mode = GPIO_MODE_AF_PP;       // 复用推挽输出
			GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_HIGH; // 高速输出
			HAL_GPIO_Init(RS485_TX_GPIO_Port, &GPIO_InitStruct);

			/* 配置 USART3 RX 引脚：PB11 → USART3_RX */
			GPIO_InitStruct.Pin = RS485_RX_Pin;           // 选择 PB11 引脚
			GPIO_InitStruct.Mode = GPIO_MODE_INPUT;       // 输入模式
			GPIO_InitStruct.Pull = GPIO_NOPULL;           // 无上下拉
			HAL_GPIO_Init(RS485_RX_GPIO_Port, &GPIO_InitStruct);

			/* 配置 USART3 中断优先级并使能 */
			HAL_NVIC_SetPriority(USART3_IRQn, 0, 0);      // 设置优先级为最高
